<template>
  <v-container>
    ...
    <div
      ...
      style="top:0; left:0; width:100vw;position:absolute;"
    >
      <v-alert
        :type="alert_type"
        dark
        ...
        dismissible
        v-on:click="alert_visible = false"
        >{{ alert_text }}</v-alert
      >
    </div>
    <!--NEW CARD-->
    <v-expand-transition>
      <div v-show="expand" style="margin:0">
        <v-row class="pt-5">
        ...
        <v-row>
          <v-col sm="12" md="8" lg="8">
            <v-row>
              <v-col sm="12" md="6" lg="6">
                <span>Select topic<span style="color:red">*</span>:</span>
                <v-combobox
                  v-model="selected.topicName"
                  :items="list_all_topics"
                  item-value="cveTopic"
                  item-text="topicName"
                  @change="select_topic"
                ></v-combobox>
              ...
              <v-col sm="12" md="6" lg="6">
                <span class=""
                  >Insert text<span style="color:red">*</span>:</span
                >
          ...
          <v-col sm="6" md="6" lg="4">
            <v-card elevation="2" min-height="310">
              <v-card-title class="justify-center">{{
                preview_card.topicName
              }}</v-card-title>
              <v-card-text>
                <v-img
                  contain
                ...
                <v-row>
                  <v-text-field
                    ...
                    readonly
                    v-model="preview_card.cardText"
                  ></v-text-field>
                </v-row>
                <v-row class="d-flex justify-center">
                  <v-icon
                    v-if="input_audio_value != '' && input_audio_value != null"
                    medium
                    border
                    >mdi-volume-high</v-icon
                  >
                  <v-icon
                    v-if="input_video_value != '' && input_video_value != null"
                    medium
                    border
                  ...
                  <v-icon
                    v-if="
                      input_meaning_value != '' && input_meaning_value != null
                    "
                    medium
                    border
    ...
    >
      <template v-slot:item="row">
        <tr>
          ...
          <td>
            <span
              v-if="row.item.pathImage"
              @click="show_dialog_image(row.item.pathImage)"
              ><v-icon border>mdi-image-album</v-icon>
            </span>
            <span
              v-if="row.item.pathSound"
              @click="show_dialog_audio(row.item.pathSound)"
              ><v-icon border>mdi-volume-high</v-icon>
            </span>
            <span
              v-if="row.item.pathVideo"
              @click="show_dialog_video(row.item.pathVideo)"
              ><v-icon border>mdi-video-box</v-icon>
            </span>
            <span
              v-if="row.item.pathMeaning"
              @click="show_dialog_meaning(row.item.pathMeaning)"
              ><v-icon border>mdi-file-pdf-box</v-icon></span
            >
          </td>
          <td>
            <v-icon medium border @click="edit_registry(row.item.cveCard)"
              >mdi-pencil-outline</v-icon
            >
            <v-icon medium border @click="delete_registry(row.item.cveCard)"
    ...
    <!--DIALOGS-->
    <DialogMeaning
      :dialog.sync="dialog_meaning_visible"
      :image_url.sync="dialog_meaning_url"
    />
    <DialogImage
      :dialog.sync="dialog_image_visible"
      :image_url.sync="dialog_image_url"
    />
    <DialogVideo
      :dialog.sync="dialog_video_visible"
      :playerOptions.sync="dialog_video_options"
    />
    <DialogAudio
      :dialog.sync="dialog_audio_visible"
      :playerOptions.sync="dialog_audio_options"
    />
  </v-container>
</template>
<script>
...
export default {
  components: { DialogMeaning, DialogImage, DialogVideo, DialogAudio },
  data() {
    return {
      expand: false,
      selected: {},
      ...
      search: "",
      headers: [
        { text: "ID", align: "start", sortable: true, value: "cveCard" },
        { text: "Topic", align: "start", sortable: true, value: "topicName" },
        { text: "Text", align: "start", sortable: true, value: "cardText" },
  ...
  methods: {
    showForm() {
      ...
      if (this.expand) {
        this.icon_new = "mdi-minus";
      } else {
        this.icon_new = "mdi-plus";
      }
    },
    async saveRegistry(e) {
      e.preventDefault();
      this.show_alert("Uploading card, wait please...", "info");
      this.btn_save = true;
      if (
        this.preview_card.topicName == "" ||
      ...
      if (this.preview_card.pathImage == "IMAGE PENDING UPLOAD") {
        ...
        if (!img) {
          ...
          this.preview_card.pathImage = img.path;
        }
      }
      ...
      if (this.input_audio_value != "" && this.input_audio_value != null) {
        if (this.preview_card.pathSound == "") {
          ...
          if (!mp3) {
            ...
            this.preview_card.pathSound = mp3.path;
          }
        }
      } else {
      ...
      if (this.input_video_value != "" && this.input_video_value != null) {
        if (this.preview_card.pathVideo == "") {
          ...
          if (!mp4) {
            ...
            this.preview_card.pathVideo = mp4.path;
          }
        }
      } else {
      ...
      if (this.input_meaning_value != "" && this.input_meaning_value != null) {
        ...
        if (this.preview_card.pathMeaning == "") {
          ...
          if (!pdf) {
            ...
            this.preview_card.pathMeaning = pdf.path;
          }
        }
      } else {
      ...
      try {
        var response=await axios.post(P.VUE_APP_INSERT_TOPIC_CARD, this.preview_card)
        if(response){
        ...
        console.log(error.response);
          if (error.response.data.code)
            this.show_alert(
              error.response.data.message + " in this topic",
              "error"
            );
          else this.show_alert("Error: " + error, "error");
      } 
    ...
    show_dialog_image(url_dialog_image) {
      let urlsrc = url_dialog_image.includes("http")
        ? url_dialog_image
        : P.VUE_APP_URL_FILES_TEMP + url_dialog_image;
      console.log(urlsrc);
    ...
    show_dialog_video(url_dialog_video) {
      (this.dialog_video_options = {
        ...
        sources: [
          {
            type: "video/mp4",
            src: url_dialog_video.includes("http")
              ? url_dialog_video
              : P.VUE_APP_URL_FILES_TEMP + url_dialog_video,
          },
        ],
      }),
    ...
    show_dialog_audio(url_dialog_audio) {
      (this.dialog_audio_options = {
        ...
        sources: [
          {
            type: "audio/mp3",
            src: url_dialog_audio.includes("http")
              ? url_dialog_audio
              : P.VUE_APP_URL_FILES_TEMP + url_dialog_audio,
          },
        ],
      }),
    ...
    async load_topics() {
      ...
      try {
        let response=await axios.get(P.VUE_APP_GET_ALL_TOPICS)  
    ...
    async upload_file(file, type) {
      try {
        ...
        var res = await axios.post(P.VUE_APP_UPLOAD_FILE, formData, {
          headers: { "Content-Type": "multipart/form-data", "X-Type": type },
        });
        console.log(res);
        if (res.status == 200) {
    ...
    show_dialog_meaning(url_dialog_meaning) {
      ...
      this.dialog_meaning_url = url_dialog_meaning.includes("http")
        ? url_dialog_meaning
        : P.VUE_APP_URL_FILES_TEMP + url_dialog_meaning;
    },
    edit_registry(id) {
      ...
      this.table_data_cards.forEach((element) => {
        if (element.cveCard == id) {
          ...
          this.selected = {
            cveTopic: element.cveTopic,
            topicName: element.topicName,
          };
          this.image_url = element.pathImage.includes("http")
            ? element.pathImage
            : P.VUE_APP_URL_FILES_TEMP + element.pathImage;
          this.input_image_value = new File(
            [element.pathImage],
            element.pathImage.split("/").pop()
          );
          if (element.pathSound != "") {
            this.input_audio_value = new File(
              [element.pathSound],
              element.pathSound.split("/").pop()
            );
          }
          if (element.pathVideo != "")
            this.input_video_value = new File(
              [element.pathVideo],
              element.pathVideo.split("/").pop()
            );
          if (element.pathMeaning != "")
            this.input_meaning_value = new File(
              [element.pathMeaning],
              element.pathMeaning.split("/").pop()
            );
    ...
    async get_all_data() {
      ...
      this.table_data_cards = await axios
        .get(P.VUE_APP_GET_ALL_CARDS)
        .then(function(res) {
          if (res.status == 200) {
            return res.data;
          } else {
      ...
      if (this.table_data_cards.length) {
        this.show_alert("Data loaded", "success");
      }
    },
    async delete_registry(id) {
      var x=confirm("Are you sure?")
      if(!x)return
      console.log(P.VUE_APP_DELETE_CARD + "?cveCard=" + id);
      var res = await axios.get(P.VUE_APP_DELETE_CARD, {
        params: { cveCard: id },
      });
      if (res.status === 200) {
  ...
  async created() {
    ...
    this.load_topics();
    console.log(P.VUE_APP_GET_ALL_CARDS);
    this.table_data_cards = await axios.get(P.VUE_APP_GET_ALL_CARDS)
      .then(function(res) {
        if (res.status == 200) {
          return res.data;
        } else {
  ...
  computed: {
    filteredItems() {
      return this.table_data_cards.filter((item) => {
        return (
          item.cardText.toLowerCase().indexOf(this.search.toLowerCase()) > -1 ||

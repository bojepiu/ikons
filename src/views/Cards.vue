        ...
        <v-row>
          <v-col sm=12 md=8 lg=8>
            <v-row>
              ...
              <v-col sm=12 md=6 lg=6 >
                <span class="">Insert text<span style="color:red">*</span>:</span>
            ...
            <v-row>
              <v-col sm=6 md=6 lg=6 class="d-flex justify-end">
                <v-btn :disabled="btn_save" depressed color="primary" class="pa-4 mt-3" @click="saveRegistry($event)">{{text_btn_save}}</v-btn>
              </v-col>
              <v-col sm=6 md=6 lg=6 class="d-flex justify-start">
                <v-btn :disabled="btn_save" depressed color="secondary" class="pa-4 mt-3" @click="cancelRegistry()">Cancel</v-btn>  
              </v-col>
              </v-row>
          </v-col>
          <v-col sm=6 md=6 lg=4>
            <v-card elevation="2" min-height="310">
              ...
              <v-card-text >
                ...
                ></v-img>
                <v-row>
                    <v-text-field class="text-align:center;" readonly v-model="preview_card.cardText"></v-text-field>
                </v-row>
  ...
  methods:{
    ...
    async saveRegistry(e){
      ...
      axios.post(P.VUE_APP_INSERT_TOPIC_CARD,this.preview_card).then(response=>{
        ...
        this.show_alert(error.response.data.message+' in this topic','error')
        else
